...
bool headerLoops(string prevHeader, const string clientIPAddress){
    vector<string> tokens;
    stringstream ss(prevHeader);
    string tok;
    while(getline(ss, tok, ',')) tokens.push_back(tok);

    for(string s: tokens){
        if(s == clientIPAddress) return true;
    }
...
void HTTPRequest::ingestHeader(istream& instream, const string& clientIPAddress) {
  ...
  if(containsName("x-forwarded-for")){
    string prevHeader = requestHeader.getValueAsString("x-forwarded-for");
    if(headerLoops(prevHeader, clientIPAddress)){
        containsLoop=true;
        return;
    }
    prevHeader = prevHeader + "," + clientIPAddress;
    requestHeader.addHeader("x-forwarded-for", prevHeader);
  } else{
